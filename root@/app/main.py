...
seen_audio_urls: Dict[str, set[str]] = {}
...
async def fetch_audio_and_queue(session_id: str, audio_url: str):
    ...
    if session_id not in seen_audio_urls:
        seen_audio_urls[session_id] = set()

    # De-duplicate retries for the same chunk URL to prevent backlog/latency buildup.
    if audio_url in seen_audio_urls[session_id]:
        print(f"Skipping duplicate audio URL for {session_id}: {audio_url}")
        return
            ...
            for url in candidates:
                async with http.get(url) as resp:
                    if resp.status == 200:
                        ...
                        seen_audio_urls[session_id].add(audio_url)
                        print(
...
async def websocket_stream(websocket: WebSocket, session_id: str):
    ...
    try:
        ...
        if session_id in seen_audio_urls:
            del seen_audio_urls[session_id]
    except Exception as e:
        print(f"Avatar stream error for session {session_id}: {repr(e)}")
